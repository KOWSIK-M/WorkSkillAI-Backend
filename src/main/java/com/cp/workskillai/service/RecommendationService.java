...
private List<CourseRecommendation> extractCoursesFromMlResponse(Map<String, Object> mlResponse) {
    try {
        ...
        if (courseData == null) {
            log.warn("No course recommendations found in ML response");
            return new ArrayList<>();
        }
        
        log.info("📚 Found {} courses in ML response", courseData.size());
        
        List<CourseRecommendation> courses = courseData.stream()
                .map(course -> {
                    try {
                        // Debug: log what we're receiving
                        log.info("📋 Course data: {}", course.keySet());
                        
                        CourseRecommendation courseRec = CourseRecommendation.builder()
                                .id((String) course.get("id"))
                                .skillId((String) course.get("skillId")) // This is from Python
                                .skillName((String) course.get("skillId")) // Use skillId as skillName since Python doesn't send skillName
                                ...
                                .studentCount(course.get("students") != null ? ((Number) course.get("students")).intValue() : 
                                             course.get("studentCount") != null ? ((Number) course.get("studentCount")).intValue() : 1000)
                        ...
                        log.info("✅ Mapped course: {} for skill: {}", courseRec.getTitle(), courseRec.getSkillName());
                        return courseRec;
                        
                    } catch (Exception e) {
                        log.error("❌ Error mapping individual course: {}", e.getMessage());
                        return null;
                    }
                })
        ...
        log.info("🎯 Successfully mapped {} courses", courses.size());
        return courses;
